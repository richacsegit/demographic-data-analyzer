...
def calculate_demographic_data(print_data=True):
    ...
    rich_percentage = (num_min_workers['age'][num_min_workers['salary']== '>50K'].count() / num_min_workers['salary'].count())*100

    # What country has the highest percentage of people that earn >50K?
    
    ptemtfk= df['native-country'][df['salary']== '>50K'].value_counts()

    total_ppl= df['native-country'].value_counts()
    
    per_g50k= (ptemtfk/ total_ppl)*100

    highest_earning_country = per_g50k[per_g50k== per_g50k.max()].index

    highest_earning_country_percentage = round(((ptemtfk/ total_ppl)*100).max(), 1)
